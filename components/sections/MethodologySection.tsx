  ...
  return (
    <section id="about" className="relative py-14 lg:py-24 bg-cetl-surface overflow-hidden">
      ...
      <Container className="relative">
        ...
        <div className="max-w-6xl mx-auto rounded-2xl bg-white border border-cetl-border shadow-[0_4px_28px_-10px_rgba(0,0,0,0.15)] p-5 sm:p-8 lg:p-10">
          {/* Pillars */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
            {m.pillars.map((pillar) => {
              ...
              return (
                ...
                >
                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 border-cetl-gold-400 flex items-center justify-center shrink-0">
                    <Icon className="w-6 h-6 sm:w-7 sm:h-7 text-cetl-navy-800" strokeWidth={1.75} />
                  </div>
                  <div>
                    <h3 className="font-display text-cetl-navy-900 font-bold text-sm sm:text-base leading-snug">
                      {pillar.title}
                    </h3>
          ...
          <div className="relative h-10 hidden sm:block" aria-hidden>
          ...
          <div className="flex justify-center sm:hidden py-3" aria-hidden>
            <ArrowDown className="text-cetl-border" size={16} strokeWidth={2.5} />
          </div>
          ...
          <div className="flex w-full rounded-lg overflow-hidden shadow-[0_2px_10px_-4px_rgba(0,0,0,0.2)]">
            {m.modules.map((mod, i) => {
              ...
              return (
                <div
                  key={mod.title}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 px-3 sm:px-6 text-white ${MODULE_BG[i]}`}
                  style={{
                    clipPath:
                      i === 0
                ...
                >
                  <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-cetl-gold-400 shrink-0" strokeWidth={1.75} />
                  <span className="font-display font-bold text-[11px] sm:text-sm text-center leading-tight">
                    {mod.title}
                  </span>
                </div>
              );
          ...
          <p className="text-cetl-text-muted text-xs sm:text-sm italic text-center mt-5 mb-8">{m.embeddedNote}</p>

          {/* Steps */}
          <div className="rounded-2xl border-2 border-dashed border-cetl-gold-400 p-5 sm:p-8">
            <div className="flex flex-wrap sm:flex-nowrap justify-center items-start gap-x-1 gap-y-8">
              {m.steps.map((step, i) => {
                const Icon = ICON_MAP[step.icon];
                return (
                  <div key={step.n} className="flex items-start">
                    <div className="flex flex-col items-center gap-2 text-center w-[104px] sm:w-[130px]">
                      <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white border-[3px] border-cetl-gold-400 flex items-center justify-center shadow-[0_2px_10px_-4px_rgba(0,0,0,0.18)]">
                      ...
                      <span className="font-display text-cetl-navy-900 text-[11px] sm:text-xs font-bold leading-tight">
                        {step.title}
                      </span>
                      <p className="text-cetl-text-muted text-[10px] leading-tight">{step.desc}</p>
                    </div>
                    {i < m.steps.length - 1 && (
                      <ChevronRight className="hidden sm:block text-cetl-gold-400 shrink-0 mt-5" size={20} strokeWidth={2.5} />
                    )}
                  </div>
                );
              })}
          ...
          {/* Outcomes */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-4 mt-10">
            {m.outcomes.map((outcome) => {
              ...
              return (
                <div key={outcome.title} className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-display text-cetl-navy-900 font-bold text-xs sm:text-sm leading-snug">
                      {outcome.title}
                    </h4>
